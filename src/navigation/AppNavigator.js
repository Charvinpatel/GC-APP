...
function CustomDrawerContent(props) {
  ...
  const isAdmin = user?.role === 'admin' || !user?.role; // Default to admin for safety, but check role

  const ALL_NAV_ITEMS = [
    { label: 'Dashboard',     icon: 'grid-outline',            screen: 'Home' },
    // Driver-only items
    { label: 'Today Trips',   icon: 'today-outline',           screen: 'DriverMyTrips', driverOnly: true },
    { label: 'Trip History',  icon: 'time-outline',            screen: 'DriverHistory', driverOnly: true },
    // Admin-only items
    { label: 'Today Trips',   icon: 'today-outline',           screen: 'VerifyTrips',   adminOnly: true },
    ...
    { label: 'Bills',         icon: 'receipt-outline',         screen: 'Bills',         adminOnly: true },
...
const headerStyles = StyleSheet.create({
  bar:     { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.surface[950], paddingTop: 52, paddingBottom: 12, paddingHorizontal: 12, borderBottomWidth: 1, borderBottomColor: colors.surface[800] },
  menuBtn: { width: 36, height: 36, alignItems: 'center', justifyContent: 'center' },
  title:   { flex: 1, textAlign: 'center', fontSize: 16, fontWeight: '700', color: colors.white },
});
